...
class openvas():
    ...
    def get_report(self, reportID, report_location):
        ...
        try:
            ...
            with open(report_location, 'w') as f:
                f.write(report_data)
            print(f"{Fore.GREEN}[+] Successfully saved report to {report_location}{Style.RESET_ALL}")
        ...
        try:
            ...
            try:
                ...
                if status_element is not None:
                    ...
                    if status == 'Done':
                        print(f"{Fore.GREEN}[✓] Scan {reportID[:8]}... Done{Style.RESET_ALL}")
                        return True
                    elif status == 'Running':
                        ...
                        print(f"{Fore.CYAN}[OpenVAS 🌿] {progress_color}Running: {progress}% complete{Style.RESET_ALL}")
                        return False
                    else:
                        print(f"{Fore.YELLOW}[OpenVAS 🌿] Status: {status}{Style.RESET_ALL}")
                        return False
                else:
            ...
            print(f"{Fore.RED}[!] Error checking scan status: {str(e)}{Style.RESET_ALL}")
    ...
    def wait_for_scan(self):
        ...
        while self.queue_scan_names:
            print(f"{Fore.CYAN}[*] Current Queue: {len(self.queue_scan_names)} scans remaining{Style.RESET_ALL}")
            ...
            for scan, report_id, report_location in zip(self.queue_scan_names,
                ...
                if done:
                    print(f"{Fore.GREEN}[+] Scan {scan} completed, retrieving report...{Style.RESET_ALL}")
                    self.get_report(reportID=report_id, report_location=report_location)
                    self.process_findings(report_location=report_location)
                else:
                    print(f"{Fore.YELLOW}[*] Scan {scan} still in progress...{Style.RESET_ALL}")
            ...
            if self.queue_scan_names:
                print(f"{Fore.BLUE}[*] Waiting 60 seconds before next check... ({len(self.queue_scan_names)} scans remaining){Style.RESET_ALL}")
                time.sleep(60)
            else:
    ...
    def process_findings(self, report_location) -> None:
        ...
        """
        print(f"{Fore.BLUE}[*] Processing findings from report: {report_location}{Style.RESET_ALL}")
        try:
        ...
        print(f"{Fore.GREEN}[+] Processed {vulnerability_count} vulnerabilities ({skipped_count} skipped as false positives){Style.RESET_ALL}")
    ...
    def run(self):
        ...
        if self.ips is None:
            print(f"{Fore.YELLOW}[!] No targets provided for OpenVAS scan. Skipping...{Style.RESET_ALL}")
            return
        
        # Check if it's a pandas Series
        if hasattr(self.ips, 'empty') and self.ips.empty:
            print(f"{Fore.YELLOW}[!] Empty target list for OpenVAS scan. Skipping...{Style.RESET_ALL}")
            return
        
        # Check if it's a pandas Series with only NaN values
        if hasattr(self.ips, 'isna') and hasattr(self.ips, 'all') and self.ips.isna().all():
            print(f"{Fore.YELLOW}[!] No valid IPv4 targets for OpenVAS scan (only NaN values). Skipping...{Style.RESET_ALL}")
            return
        ...
        if not target_list:
            print(f"{Fore.YELLOW}[!] No valid IPv4 targets for OpenVAS scan after filtering. Skipping...{Style.RESET_ALL}")
            return
        
        print(f"{Fore.GREEN}[+] Starting OpenVAS scan on {len(target_list)} targets{Style.RESET_ALL}")
